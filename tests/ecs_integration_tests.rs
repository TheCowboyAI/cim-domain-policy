...
use cim_domain_policy::components::policy::{PolicyStatus, PolicyType, ScopeType};
...
fn test_create_policy_system() {
    ...
    let policy_id = Uuid::new_v4();
    world
        .resource_mut::<Events<PolicyCreated>>()
        .send(PolicyCreated {
            policy_id,
...
fn test_approval_workflow() {
    ...
    // Submit for approval
    world
        .resource_mut::<Events<PolicySubmittedForApproval>>()
        .send(PolicySubmittedForApproval {
            policy_id,
    ...
    world
        .resource_mut::<Events<ApprovalStepEvent>>()
        .send(ApprovalStepEvent {
            approval_id,
            step_id: Uuid::new_v4(),
...
fn test_enforcement_system() {
    ...
    world
        .resource_mut::<Events<EnforcementRequest>>()
        .send(EnforcementRequest {
            policy_id,
...
fn test_violation_recording() {
    ...
    world
        .resource_mut::<Events<ViolationDetected>>()
        .send(ViolationDetected {
            policy_id,
...
fn test_policy_lifecycle_flow() {
    ...
    // Update policy
    world
        .resource_mut::<Events<PolicyUpdated>>()
        .send(PolicyUpdated {
            policy_id,
    ...
    // Activate policy
    world
        .resource_mut::<Events<PolicyActivated>>()
        .send(PolicyActivated { policy_id });
